# Memory Index

## User Profile
- [user_academic_researcher.md](user_academic_researcher.md) - Onour Impram: PhD candidate (Istanbul Uni), licensed psychologist (TR/GR/IE), author of "Üretken Yapay Zekâ ve Ruh Sağlığı" (Nobel). Greek citizen in Istanbul. Married to Betül. Hezarfen/TheGoatPsy/keçipsikologu. Expert in AI-behavioral addiction, ACoR theory, narrative therapy, positive psychology.

## Working Preferences
- [feedback_working_style.md](feedback_working_style.md) - No dashes/semicolons/emojis. Turkish preferred. APA 7 default. PhD-level rigor. Autonomous execution. Publication-ready outputs. Licensed professional (no hedging needed).

## Active Projects
- [project_active_manuscripts.md](project_active_manuscripts.md) - Book published (Nobel). HPR Model (JBA resubmission). Neo Pygmalion (SR). ACoR Theory (Nature/Lancet). Narrative Therapy SR (completed). Dublin ECPP July 2026. ECADOC 2026 applied.
- [project_student_pack_integrations.md](project_student_pack_integrations.md) - GitHub Student Pack: 15 tools integrated. CLIs: Doppler, Sentry, gh, doctl, Heroku. SDKs: sentry-sdk, newrelic, datadog, configcat. Hub: python/hzf_integrations.py. Verify: scripts/verify_integrations.py.

## External References
- [reference_external_systems.md](reference_external_systems.md) - GitHub: TheGoatPsy. Instagram: @kecipsikologu. ORCID: 0000-0003-1076-3928. Trading: Pepperstone/MT5. AI: Hezarfen ecosystem. Orgs: Yeşilay, Enstitü Sosyal, Salus, PSI Ireland.

## Archived
- [feedback_credential_warning.md](feedback_credential_warning.md) - Warned about sharing credentials with AI tools
